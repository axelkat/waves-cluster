"""
crosser.py - clustering's role in transport. 
Data for fig. 2.A,B,C,D.
  ...
  fig3a.csv   Per-config arrival cluster fraction. Of the particles that reach
              ...
              persistent cluster (majority-of-displacement rule).
  ...
  fig3bb.csv  ARRIVAL cluster velocity distribution. For each crosser, the
              ...
              labelled "clustered" by the displacement-majority rule. The histogram
...
Note: Accumulation is deliberately NOT a bare y >= Y_BULK_LIMIT cut. A persistent
cluster whose leading member reaches Y_CLUSTER_CONTACT drags its whole
membership into the accumulation zone.
"""
...
def build_frames(df_v, df_nodes_v, df_persist_v):
    ...
    df = df.merge(
        df_nodes_v[['processed_frame_index', 'fixed_id', 'cluster_id']],
        on=['processed_frame_index', 'fixed_id'], how='left'
    )
    df = df.merge(
        df_persist_v[['processed_frame_index', 'fixed_id', 'clustered_eff']],
        on=['processed_frame_index', 'fixed_id'], how='left'
    )
...
def find_crossings(df):
    ...
    df['crossed_standard'] = df.groupby('fixed_id')['above_threshold'].diff().fillna(0) > 0
    ...
    if not triggered.empty:
        members = eff.merge(
            triggered, on=['processed_frame_index', 'cluster_id'], how='inner'
        )[['processed_frame_index', 'fixed_id']].drop_duplicates()
        ...
        key = members[['prev_frame', 'fixed_id']].rename(
        ...
        df = df.drop(columns=['crossed_propagated']).merge(
            key, on=['processed_frame_index', 'fixed_id'], how='left'
        )
        df['crossed_propagated'] = df['crossed_propagated'].fillna(False)

    df['crossed'] = df['crossed_standard'] | df['crossed_propagated']
    crossed_df = df[df['crossed']].drop_duplicates(subset=['fixed_id'], keep='first').copy()
    if crossed_df.empty:
        return pd.DataFrame(), pd.DataFrame()
